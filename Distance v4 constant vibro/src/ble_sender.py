...
def map_distance_to_intensity(distance_mm):
    """
    Rechnet Distanz in Vibration um, basierend auf der Regel:
    -1 Vibrationsstufe pro +1 mm Abstand.
    """
    MIN_DISTANCE = 120  # 12 cm: Maximale Vibration (255)

    # Gemäß der Regel (255 Stufen * 1 mm/Stufe = 255mm Reichweite)
    # hört die Vibration bei 120mm + 255mm = 375mm auf.
    MAX_DISTANCE = 375  # 37,5 cm
    ...
    # Direkte lineare Umrechnung:
    # Für jeden Millimeter, den die Distanz größer als MIN_DISTANCE ist,
    # wird die Intensität um 1 verringert.
    intensity = 255 - (distance_mm - MIN_DISTANCE)

    return int(max(0, min(255, intensity)))
