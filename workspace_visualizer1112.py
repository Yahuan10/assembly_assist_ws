...
ZONE_LAYOUT: Dict[str, Dict[str, Tuple[int, ...]]] = {
    "Battery": {"rect": (520, 60, 710, 280), "label": "Battery"},

    "Motor": {"rect": (200, 80, 510, 280), "label": "Motor"},

    "Assembly": {"rect": (250, 595, 750, 745), "label": "Assembly"},


    "PCB1": {"rect": (565, 315, 790, 375), "label": "PCB1"},
    "PCB2": {"rect": (225, 308, 565, 385), "label": "PCB2"},  

    "PCB3": {"rect": (60, 460, 225, 640), "label": "PCB3"},
...
# Robot base indicator (used for fallback linear projection only)
...
class SchedulerVisualizer:
    ...
    def __init__(self) -> None:
        ...
        self.live_tcp_max_age: float = float(rospy.get_param("~live_tcp_max_age", 5.0))
        self._live_tcp_path: Deque[Tuple[int, int]] = deque(maxlen=history_len)
        self._live_tcp_times: Deque[float] = deque(maxlen=history_len)
        ...
        cv2.setWindowProperty("Subie Workbench Overview",cv2.WND_PROP_FULLSCREEN,cv2.WINDOW_FULLSCREEN)
    ...
    def _draw_workbench(self, canvas: np.ndarray, current_state: str, next_state: Optional[str]) -> None:
        ...
        for name, data in ZONE_LAYOUT.items():
            if name == "Handover" and not self.show_handover_zone:
                continue
            if name == "Home" and not self.show_home_zone:
                continue
    ...
    def _draw_status_panel(
        ...
        # Ready indicator
        ready_rect = (850, 80, 1010, 130)
        cv2.rectangle(canvas, (ready_rect[0], ready_rect[1]), (ready_rect[2], ready_rect[3]), (255,255,255), 2)
        cv2.putText(canvas, "Ready:", (ready_rect[0] + 10, ready_rect[1] + 30), FONT, 0.8, (255,255,255), 2, cv2.LINE_AA)
        ...
        cv2.putText(canvas, "Current", (panel_rect[0] + 10, panel_rect[1] + 25), FONT, 0.6, (255,255,255), 1, cv2.LINE_AA)
        cv2.putText(
            canvas,
    ...
    def _append_live_point(self, point: Tuple[int, int], timestamp: float) -> None:
        with self._lock:
            self._live_tcp_path.append(point)
            self._live_tcp_times.append(timestamp)
            self._prune_live_tcp(timestamp)

    def _prune_live_tcp(self, now: float) -> None:
        max_age = max(0.0, self.live_tcp_max_age)
        while self._live_tcp_times and now - self._live_tcp_times[0] > max_age:
            self._live_tcp_times.popleft()
            self._live_tcp_path.popleft()
    ...
    def _sample_rtde_tcp_pose(self) -> bool:
        ...
        self._append_live_point(point, now)
        return True
    ...
    def _draw_tcp_planned_path(self, canvas: np.ndarray) -> None:
        ...
        cv2.putText(canvas, "TCP Plan", (label_x, label_y), FONT, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
    ...
    def _draw_live_tcp_path(self, canvas: np.ndarray) -> None:
        ...
        latest = pts[-1]
        cv2.circle(canvas, latest, 8, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(
            canvas,
            "TCP",
